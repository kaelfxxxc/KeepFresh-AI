-- ============================================================================
-- KeepFresh AI — Subscriptions, Entitlements, Storage Areas & Inventory Ops
-- ============================================================================
-- Adds the subscription / entitlement layer from the project specification:
--
--   * subscription_plans / feature_entitlements  catalogue + capability matrix
--   * user_subscriptions / subscription_usage    per-user entitlement state
--   * storage_areas                              multi-area inventory
--   * inventory_transactions / price_history     audit trail + price tracking
--   * organizations / organization_members       staff, roles, bulk inventory
--
-- Everything here is idempotent — safe to run more than once.
--
-- SECURITY MODEL (read before changing anything)
--
--   1. Limits are enforced in the DATABASE first, by triggers and SECURITY
--      DEFINER functions. The client re-checks only so it can show a friendly
--      upgrade prompt instead of a raw error. A user with a patched client
--      still cannot exceed their plan.
--
--   2. A plan can never be activated from the client. `user_subscriptions` is
--      written by SECURITY DEFINER routines (the trial grant on signup and the
--      verified-payment webhook) and `guard_subscription_update()` rejects any
--      client-side change to plan_id / status / billing period.
--
--   3. Payment receipts and purchase tokens live in
--      `subscription_provider_receipts`, which has RLS enabled and ZERO
--      policies — only the service role can read or write it. The Expo bundle
--      therefore never holds a service-role key or another user's receipt.
--
--   4. Plan limits are NOT tied to `profiles.account_type`. A Household user
--      may buy Food Establishment Pro; entitlements follow the ACTIVE
--      subscription, and account_type only decides which trial they start on.
--
