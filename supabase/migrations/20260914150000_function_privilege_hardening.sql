-- ============================================================================
-- KeepFresh AI — Function privilege hardening
-- ============================================================================
-- Postgres grants EXECUTE on a new function to PUBLIC by default, which in
-- Supabase means the `anon` role can reach it through PostgREST's /rpc endpoint
-- even though it has no session and no user id.
--
-- That matters for the SECURITY DEFINER routines added in the previous
-- migrations. They bypass row-level security by design, and several of them
-- branch on `auth.uid() IS NULL` to mean "this is a server-side call". An `anon`
-- caller also has a NULL auth.uid(), so without this migration an unauthenticated
-- request could reach those branches — for example `adjust_inventory_quantity`,
-- whose ownership check is skipped on that path.
--
-- The fix is the grant layer, where it belongs: `anon` and PUBLIC lose EXECUTE
-- on every routine below, and only `authenticated` (a real signed-in session)
-- and `service_role` (our own Edge Functions) keep it. From there the rule
-- inside each function holds:
--
--   auth.uid() IS NOT NULL  ->  a user's session; ownership is enforced
--   auth.uid() IS NULL      ->  only reachable as service_role, which is ours
--
-- Trigger functions are included for defence in depth. Revoking EXECUTE on a
-- trigger function does not affect the trigger — Postgres fires triggers
-- without a privilege check on the function.
-- ============================================================================
